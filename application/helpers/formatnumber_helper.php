...
function n2y($value) {$alphabet = range('A', 'Z'); return (!is_numeric($value)) ? (intVal(array_search($value, $alphabet)) + 2019) : $alphabet[intVal($value)-2019] ;}
...
function getPartyListOption($partyList,$partyId = 0){
	...
	foreach($partyList as $row):
		...
		$options .= '<option value="'.$row->id.'" data-sales_executive="'.$row->executive_id.'" data-discount_structure="'.$row->discount_structure.'" '.$selected.'>'.$row->party_name.'</option>';
	endforeach;
...
function getItemListOption($itemList,$itemId = 0){
	...
	foreach($itemList as $row):
		...
		$item_name = (!empty($row->full_name))?$row->full_name:$row->item_name;
		$itemName = (!empty($row->item_code))?"[ ".$row->item_code." ] ".$item_name : $item_name;
		$options .= '<option value="'.$row->id.'" data-item_code="'.$row->item_code.'"  data-gst_per="'.$row->gst_per.'" data-wt_pcs="'.$row->wt_pcs.'" data-primary_packing="'.$row->primary_packing.'" data-price="'.$row->price.'"  data-master_packing="'.$row->master_packing.'" data-category_id="'.$row->category_id.'" '.$selected.'>'.$itemName.'</option>';
	endforeach;
