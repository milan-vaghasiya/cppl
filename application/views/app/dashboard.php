        ...
        <div class="container">
			...
			<div class="service-area mb-4">
				<div class="service-box">
					...
					<h5 class="mb-0"><?=(!empty($logCount->new_lead)?$logCount->new_lead:0)?></h5>
				</div>
				<div class="service-box">
					<div class="dz-icon mx-auto mb-2">
					...
					<h5 class="mb-0"><?=!empty($logCount->new_order)?$logCount->new_order:0?></h5>
				</div>
				<div class="service-box" onclick="window.location.href='<?=base_url($headData->controller.'/reminderList')?>'">
					<div class="dz-icon mx-auto mb-2">
			...
			<div class="card  light   bg-primary">
				<div class="card-body">
					<?php
					 ...
					 $sales_amount = (!empty($tagetData->sales_amount)?formatNumber($tagetData->sales_amount):0);
					 $leadAcheive = (!empty($tagetData->achieve_new_lead)?$tagetData->achieve_new_lead:0);
					 $salesAcheive = (!empty($tagetData->achived_amount)?formatNumber($tagetData->achived_amount):0);
					 
					$visit_achieved = (!empty($tagetData->achieve_new_visit) ? $tagetData->achieve_new_visit : 0);
					$visit_data = (!empty($tagetData->new_visit) ? $tagetData->new_visit : 0);
				   
					 $leadRatio = 0;$salesRatio = 0;$visitRation = 0;
					 if($leadAcheive > 0 && $total_new_lead > 0){ $leadRatio = (($leadAcheive*100)/$total_new_lead); }
					 if($salesAcheive > 0 && $salesAcheive > 0 && $sales_amount > 0){ $salesRatio = round((($salesAcheive*100)/$sales_amount),2); }
					 if($visit_achieved > 0 && $visit_data > 0){ $visitRation = round((($visit_achieved*100)/$visit_data),2); }
					?>
			...
			<div class="card  light badge-warning">
				<div class="card-body">
					<div class="d-flex align-items-center mb-0">
						<div class="top-area">
							<h3 class="quantity"><?=$salesAcheive.'/'.$sales_amount?></h3>
			...
			<div class="card  light badge-secondary">
				<div class="card-body">
					<div class="d-flex align-items-center mb-0">
						<div class="top-area">
							<h3 class="quantity"><?= $visit_achieved.'/'.$visit_data;?></h3>
					...
					<div class="bottom-area">
						<h6 class="review-title text-end justify-content-end"><?=$visitRation?>%</h6>
						<div class="progress">
							<div class="progress-bar progress-animated progress-bar-striped border-0 warning" style="width: <?=$visitRation?>%;" role="progressbar">
								<span class="sr-only"><?=$visitRation?>% Complete</span>
							</div>
						</div>
					</div>
				</div>
