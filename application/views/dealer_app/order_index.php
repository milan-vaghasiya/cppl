...
<div class="offcanvas offcanvas-bottom m-3 rounded" tabindex="-1" id="approveOrder" aria-modal="true" role="dialog">
    <div class="offcanvas-body small">
        <form id="approve_order">
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="">
                        <button type="button" class="btn btn-success btn-round btn-outline-dashed btnSave btn-block saveApproveOrder">Approve</button>
                    </div>
                </div>        
            </div>
        </form>
...
$(document).ready(function(){
    ...
    $(document).on('click','.saveApproveOrder',function(){
    	var formId = "approve_order";
    	var form = $('#'+formId)[0];
    	var fd = new FormData(form);
    	$(".btnSave").attr("disabled", true);
    	$.ajax({
    		url: base_url + controller + '/saveApproveOrder',
    		data:fd,
    		type: "POST",
    		global:false,
    		processData:false,
    		...
    		{
    			$('#'+formId)[0].reset(); $('#approveOrder').removeClass('show');
...
function loadData(status){
	$(".message-list").html("");	
	$.ajax({
		url: base_url  + 'dealer_app/order/getOrderData',
		data:{'status':status},
		type: "POST",
		dataType:"json",
	}).done(function(response){
...
function trash(data){
	...
	Swal.fire({
		...
		if (result.isConfirmed)
		{
			...
			window.location = base_url + 'dealer_app/order';
		}
	});
}
</script>