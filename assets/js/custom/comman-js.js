$(document).ready(function(){
	...
	$(document).on('click',".addNew",function(){
		...
		var fnJson = "{'formId':'"+formId+"','controller':'"+controllerName+"','fnsave':'"+fnsave+"','table_id':'"+table_id+"'}";		
	...
	$(document).on('change',".state_list",function(){
		...
		if(id == ""){
			$("#"+district_id).html('<option value="">Select District</option>');
			$("#"+district_id).select2();//.comboSelect();
		}else{
			$.ajax({
				...
				success:function(data){
					...
					if(selected_district != ""){
						...
						$(".district_list").trigger('change'); // 14-03-2024
					}				
				}
			});
		}
	...
	$(document).on('change',".district_list",function(){
		...
		if(id == ""){
			...
			$("#"+statutory_id).select2();//.comboSelect();
		}else{
			$.ajax({
	...
	$(document).on('change click',".itemDetails",function(){
		...
		if(item_id){
			$.ajax({
				url : base_url + 'finishGoods/getItemDetails',
				type:'post',
				data: {id:item_id},
				dataType : 'json',
			}).done(function(response){
	...
	/**** Call India Mart API ***/
	//setInterval(function(){syncTPLeads()},(60*60000));
...
function initSelect2(modal_id="")
{
	if(modal_id !== "")
	{
		$('.modal-select2').each(function (i, obj) { if ($(obj).hasClass("select2-hidden-accessible")){$(obj).select2('destroy'); } });
		$('.modal-select2').select2({ placeholder : "Select",allowHtml: true,width: null,dropdownParent: $('#'+modal_id)}); 
		$('select.modal-select2').on('select2:close', function() {
			let select = $(this);
			let totalOpt = $(this).find('option').length;
			let maxOpt = $(this).data('max_opt') || 1;
			$(this).next('span.select2').find('ul').html(function() {
				let selected = select.select2('data').length;
				if (selected > maxOpt){return "<li class='selectText'>Selected " + selected + " of " + totalOpt +"  items</li>";}
			});
		});
	} 
	else
	{
	    var modalId = $(".modal-select2").closest('.modal').attr('id');
		$('.select2').each(function (i, obj) { if ($(obj).hasClass("select2-hidden-accessible")){$(obj).select2('destroy'); } });
		$('.select2').select2({ placeholder : "Select",allowHtml: true,width: null});
		
		
		$('.modal-select2').each(function (i, obj) { if ($(obj).hasClass("select2-hidden-accessible")){$(obj).select2('destroy'); } });
		$('.modal-select2').select2({ placeholder : "Select",allowHtml: true,width: null,dropdownParent: $('#'+modalId)}); 
	...
	$.ajax({
		...
		if(!$.isEmptyObject(partyList)){
			$.each(partyList,function(index,row){  
				html += '<option value="'+row.id+'" data-discount_structure="'+row.discount_structure+'">'+row.party_name+'</option>';
			});
...
function getItemList(postData){
	$.ajax({
		url : base_url + 'finishGoods/getItemList',
		type : 'post',
		data : postData,
		dataType : 'json',
	}).done(function(response){
...
function store(postData){
	...
	$.ajax({
		...
		if(data.status==1){
			...
			Swal.fire({ icon: 'success', title: data.message}).then((result) => {if(tableId != ""){window.location.reload();}else{initTable();}});
		}else{
			...
			$.ajax(ajaxParam).done(function(response){
				if(resFunctionName != ""){
					...
					if(response.status==1){
						if(formId != ""){$('#'+formId)[0].reset(); colseModal(formId);}
						Swal.fire( 'Success', response.message, 'success' ).then((result) => {initTable();});
					}else{
			...
			$.ajax({
				...
				if(resFunctionName != ""){
					...
					if(response.status==0){
						...
						Swal.fire( 'Deleted!', response.message, 'success' ).then((result) => {if(tableId != ""){window.location.reload();}else{initTable();}});
					}	
				}
			});
...
function resPartyMaster(response,formId){
	if(response.status==1){
		...
		getPartyList({"party_type":($("#party_id").data('party_type') || 1)});
		setTimeout(function(){
...
// Used to Open Modal With Ajax Call
...
// Used to get Geo Location
function getGeoLatLong(){
	var latlong = {};
...
/*** Call Indiamart Leads ***/
function syncTPLeads(){
	
	$.ajax({
		url: base_url + 'parties/syncTPLeads',
		data:{},
		type: "POST",
		dataType:"json",
		global:false
